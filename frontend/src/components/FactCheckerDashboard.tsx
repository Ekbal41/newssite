...
import { CheckCircle, XCircle, Search, ChevronLeft, ChevronRight, Eye, Calendar, User, MapPin, ShieldCheck, X } from "lucide-react";
    ...
    return (
        <div className="space-y-8">
            <header className="space-y-2">
                ...
                <h1 className="text-3xl font-serif font-bold tracking-tight">Verification Queue</h1>
                <p className="text-muted-foreground">Review pending reports for accuracy and bias.</p>
            </header>
            ...
            {isLoading ? (
                ...
                <Card className="text-center py-12 border-dashed">
                    <CardContent>
                        ...
                        <p className="text-muted-foreground">All caught up! No articles pending verification.</p>
                    </CardContent>
                </Card>
            ) : (
                <div className="space-y-4">
            ...
            <Dialog open={!!selectedArticle} onOpenChange={(open) => { if (!open) setSelectedArticle(null); }}>
                <DialogContent className="max-w-3xl max-h-[90vh] flex flex-col">
                    <DialogHeader>
                        <DialogTitle className="font-serif text-2xl">{selectedArticle?.headline}</DialogTitle>
                        <DialogDescription>
                            By {selectedArticle?.author.name} • {selectedArticle?.location} • {dayjs(selectedArticle?.createdAt).format("MMM D, YYYY")}
                        </DialogDescription>
                    </DialogHeader>
                    <ScrollArea className="flex-1 pr-4">
                        <div className="prose prose-sm max-w-none" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(selectedArticle?.body || "") }} />
                        {selectedArticle?.sources?.length > 0 && (
                            <div className="mt-6 pt-4 border-t">
                                <h4 className="font-bold text-sm mb-2">Sources ({selectedArticle?.sources.length})</h4>
                                <ul className="text-sm space-y-1">
                                    {selectedArticle?.sources.map((s: any, i: number) => (
                                        <li key={i}>{s.name} {s.url && <a href={s.url} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">[link]</a>}</li>
                                    ))}
                                </ul>
                            </div>
                        )}
                    ...
                    <div className="pt-4 border-t space-y-4">
                        <Textarea placeholder="Verification notes (optional)..." value={comment} onChange={(e) => setComment(e.target.value)} />
                        <DialogFooter className="gap-2 sm:gap-0">
                            <Button variant="outline" onClick={() => handleReview("REJECTED")} disabled={reviewMutation.isPending}>
