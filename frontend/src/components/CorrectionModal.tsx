...
export const CorrectionModal = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      await api.post(`/articles/${article.id}/correction`, {
        reason,
        newHeadline,
        newBody,
      });
      ...
      <DialogContent className="flex flex-col !max-w-5xl w-full">
        <DialogHeader>
          <DialogTitle className="font-serif flex items-center gap-2">
            <Edit3 className="h-5 w-5" /> Apply Correction
          </DialogTitle>
          <DialogDescription>
            Modify the article and document the reason for the change.
          </DialogDescription>
        </DialogHeader>
        <ScrollArea className="pr-4 h-[70vh]">
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input
                placeholder="e.g., Corrected date of event"
                value={reason}
